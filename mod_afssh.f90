...
subroutine vv_decoherence(dtc)
 ...
 do i=1,nclass
    delr(:,:,i)=delr(:,:,i)-delr(state,state,i)
    delp(:,:,i)=delp(:,:,i)-delp(state,state,i)
  enddo
...
subroutine velocity_adjust(state_tentative,ifrust)
  ...
  if(sum(dp*dp)==0.d0.or.abs(cc)<2.d0*wave_to_J) then
    !! Trivial crossing!! CAREFUL
    !! Assuming small dtc, changing state to state_tentative without changing
    !velocity
    state=state_tentative
    ifrust=0
  else
...
subroutine write_average
  ...
  do i=1,nsteps/nstep_avg
   ...
   ! enddo
   write(100,'(21f15.7)')(i-1)*nstep_avg*dtc*1.d15,real(rho(1,1,i)),real(rho(2,2,i)),real(rho(3,3,i)),real(rho(4,4,i)),real(rho(5,5,i)),real(rho(6,6,i)),real(rho(7,7,i))
  enddo
...
subroutine tise
  ...
  do i=1,nquant
    if(abs(sum(si_adiab(:,i)*si_adiab_prev(:,i)))<1d-10) then
      call trivial
      exit
    endif
  enddo
...
subroutine trivial
  ...
  real*8 epsilon
  integer i
  real*8 si_eps(nquant,nbasis)
  real*8 Hamil(nbasis,nbasis),ens(nbasis),vect(nbasis,nquant)

  epsilon=1.d-10*wave_to_J
  Hamil=Hamil_diab+epsilon

  call diag(Hamil,nbasis,ens,vect,nquant)

  do i=1,nquant
    si_eps(:,i)=vect(:,i)
    if(sum(si_eps(:,i)*si_adiab_prev(:,i))<0.d0)si_eps(:,i)=-si_eps(:,i)
    if(sum(si_adiab(:,i)*si_eps(:,i))<0.d0)si_adiab(:,i)=-si_adiab(:,i)
  enddo
...
subroutine setup_parameters
  ...
  g_coup=dsqrt(2*lambda_B*mass(1)*omg_B**2)

 
  Hamil_site=0.d0
  Hamil_site(1,1)=505.d0
  Hamil_site(2,2)=425.d0
  Hamil_site(3,3)=195.d0
  Hamil_site(4,4)=375.d0
  Hamil_site(5,5)=600.d0
  Hamil_site(6,6)=515.d0
  Hamil_site(7,7)=465.d0

  Hamil_site(1,2)=-94.8d0; Hamil_site(2,1)=Hamil_site(1,2)
  Hamil_site(2,3)=29.8d0; Hamil_site(3,2)=Hamil_site(2,3)
  Hamil_site(3,4)=-58.9d0; Hamil_site(4,3)=Hamil_site(3,4)
  Hamil_site(4,5)=-64.1d0; Hamil_site(5,4)=Hamil_site(4,5)
  Hamil_site(5,6)=89.5d0; Hamil_site(6,5)=Hamil_site(5,6)
  Hamil_site(6,7)=35.1d0; Hamil_site(7,6)=Hamil_site(6,7)

  Hamil_site(1,3)=5.5d0; Hamil_site(3,1)=Hamil_site(1,3)
  Hamil_site(1,4)=-5.9d0; Hamil_site(4,1)=Hamil_site(1,4)
  Hamil_site(1,5)=7.1d0; Hamil_site(5,1)=Hamil_site(1,5)
  Hamil_site(1,6)=-15.1d0; Hamil_site(6,1)=Hamil_site(1,6)
  Hamil_site(1,7)=-12.2d0; Hamil_site(7,1)=Hamil_site(1,7)
  Hamil_site(2,4)=7.6d0; Hamil_site(4,2)=Hamil_site(2,4)
  Hamil_site(2,5)=1.6d0; Hamil_site(5,2)=Hamil_site(2,5)
  Hamil_site(2,6)=13.1d0; Hamil_site(6,2)=Hamil_site(2,6)
  Hamil_site(2,7)=5.7d0; Hamil_site(7,2)=Hamil_site(2,7)
  Hamil_site(3,5)=-1.2d0; Hamil_site(5,3)=Hamil_site(3,5)
  Hamil_site(3,6)=-9.3d0; Hamil_site(6,3)=Hamil_site(3,6)
  Hamil_site(3,7)=3.4d0; Hamil_site(7,3)=Hamil_site(3,7)
  Hamil_site(4,6)=-17.4d0; Hamil_site(6,4)=Hamil_site(4,6)
  Hamil_site(4,7)=-62.3d0; Hamil_site(7,4)=Hamil_site(7,4)
  Hamil_site(5,7)=-4.6d0; Hamil_site(7,5)=Hamil_site(7,5)
...
subroutine compute_potential(H_diab,delV_dels)
  ...
  H1=sum(0.5*mass*omg_B**2*x**2)
  dH1=mass*omg_B**2*x

  do i=1,nquant
    H_diab(i,i)=H_diab(i,i)+H1+g_coup*x(i)
    delv_dels(i,i,:)=dh1
